...
@task(log_prints=True)
def save_df_as_pq(df, country_code, state_code):
    venue_save_path = Path("./data/parquet/venues")
    utc_in_seconds = timegm(datetime.utcnow().timetuple())
    file_name = f"{utc_in_seconds}_{country_code}_{state_code}.parquet.gzip"
    save_path = venue_save_path / file_name
...
def update_venues(country_code="CA", state_code="AB"):
    ...
    save_df_as_pq(df=venues_df, country_code=country_code, state_code=state_code)
